let superHeroes = `[{
    ...
    "universe": "DC Comics",
    "alterEgo": "Брюс Уэйн",
    "lineOfWork": "борец с преступностью, филантроп, миллиардер",
    ...
    "extraInfo": "По популярности человек-летучая мышь может сравниться только с Суперменом. Его образ кажется очень мистическим и мрачным: черный костюм с развевающимся плащом, устрашающий Бэтмобиль, штаб-квартира в сырой пещере. Его биография настолько же темна, как и образ. В детстве у него на глазах убили родителей, и юный Брюс поклялся всеми силами защищать родной Готэм от преступности. Брюс знаменит, несмотря на пожилой возраст. Первый выпуск с его участием вышел аж в 1939 году. С каждым годом популярность только крепнет. Большую роль в этом сыграли экранизации: «Темный рыцарь», сериал «Готэм» и несколько одноименных картин. Главная причина такой любви среди фанатов — мысль, что каждый может стать Бэтменом. Если подумать, это один из немногих героев, кто добился своего положения за счет ума и упорства, а не за счет мутаций.",
    "rating": "" 
},{
    "name": "Супермен",
    "universe": "DC Comics",
    "alterEgo": "Кларк Кент",
    "lineOfWork": "борец за справедливость",
...
document.addEventListener('DOMContentLoaded', function(event) {
    ...
    for (let hero of JSON.parse(superHeroes)) {
        heroContent += `<div class="container">
            <h2>${hero.name}</h2>
            <div>Вселенная: ${hero.universe}</div>
            <div>Альтер эго: ${hero.alterEgo}</div>
            <div>Род деятельности: ${hero.lineOfWork}</div>
            <div>Друзья: ${hero.friends}</div>
            <div>Суперсилы: ${hero.superpowers}</div>
            <img src="${hero.photo}"> 
            <div>Подробнее: ${hero.extraInfo}</div></div>` +
            `<div class="rate">Поставьте оценку от 1 до 10: <input type="number" value="${hero.rating}">
            <button>OK</button>
        </div>`;
    }
...
let rates = document.querySelectorAll('input');
    ...
    for (let rate of rates) {
        if (rate.value != null) {
            localStorage.setItem('rates', JSON.stringify(rates.push(value)))
        }
    }
